...
async def profiling_summary(
    ...
    return build_progressive_profile_summary(current_user.user_id)
...
async def activate_profiling_goal(
    ...
    activate_goal(current_user.user_id, payload.type, payload.commitment_amount)
    return ActivateGoalResponse(
        summary=build_progressive_profile_summary(current_user.user_id)
    )
