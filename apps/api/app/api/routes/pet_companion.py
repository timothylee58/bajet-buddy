...
async def get_profile(current_user: AuthenticatedUser = Depends(get_current_user)) -> PetProfile:
    data = pet_companion_service.get_or_create_profile(current_user.user_id)
...
async def select_species(
    ...
    return await pet_companion_service.select_species(current_user.user_id, payload.species)
...
async def award_xp(
    ...
    return await pet_companion_service.award_xp(current_user.user_id, payload.event, payload.amount_rm)
...
async def get_nudge(
    ...
    return await pet_companion_service.generate_nudge(
        current_user.user_id, payload.context, payload.amount_rm, payload.category
    )
...
async def get_status(
    ...
    return await pet_companion_service.get_status(current_user.user_id, context, amount_rm, category)
